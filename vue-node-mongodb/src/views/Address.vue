<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex == index}" @click="checkIndex=index;selectedAddrId=item._id">
                    <dl>
                    	 <dt>{{selectedAddrId}}</dt>
                      <dt>{{item.userName}}</dt>
                  ...
                  <li class="addr-new" @click="showaAddrModal">
            ...
            <div class="next-btn-wrap">
              <!-- a标签跳转 -->
              <router-link class="btn btn--m btn--red" v-bind:to="{path:'/orderConfirm',query:{'_id':selectedAddrId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <!-- 模态框 -->
      <modal :mdShow="isMdShow" @close="closeModal('isMdShow')">
        <p slot="message">
          您是否确认要删除此地址?
        </p>
        <div slot="btnGroup">
      ...
      <!-- 修改地址  -->
      <modal :mdShow="addAShow" @close="closeModal('addAShow')">
        <ul slot="message" class="addrGruop">
        	<li>
                <input type="text" tabindex="1" name="userName" v-model="userName" value="请输入" />        		
        	</li>
        	<li >
                <input type="text" tabindex="2" name="streetName" v-model="streetName" value="请输入" />        		
        	</li>
        	<li >
                <input type="text" tabindex="3" name="postCode" v-model="postCode" value="请输入" />        		
        	</li>
        	<li >
                <input type="text" tabindex="4" name="tel" v-model="tel" value="请输入" />        		
        	</li>
        </ul>
        <div slot="btnGroup">
      ...
      addAddress(){
  		axios.post("/users/addAddress",{
				...
				if(res.status == "0"){
					this.addAShow=false;
					this.userName='请输入名字'
			        this.streetName='请输入收货地址'
			        this.postCode='请输入邮政编码'
			        this.tel='请输入电话号'
					this.init();  // 重新渲染地址列表
				}else{
					//this.errorTip=true
...
<style scoped>
.addrGruop input{ width: 100%; margin: 5px 0;}
</style>
